...
int 
cmp_index(linkedlist_t * l, node_t * n1, node_t * n2)
{
	if (n1 == n2)
	{
		return 0;
...
_linkedlist_qsort (linkedlist_t * l, int ( cmp_elements)(void * e1, void * e2), node_t * begin, node_t * end)
{
	...
	while(cmp_index(l, lbound, rbound) <= 0 )
	{
		while(cmp_index(l, lbound, end) <= 0 && cmp_elements(lbound->element, pivot->element) <= 0)
			lbound = lbound->right;

		while(cmp_index(l, begin, rbound) < 0  && cmp_elements(pivot->element, rbound->element) < 0 )
			rbound = rbound->left;

		if (cmp_index(l, lbound, rbound) <= 0)
		{
