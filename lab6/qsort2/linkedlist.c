...
_linkedlist_qsort (linkedlist_t * l, int ( cmp_elements)(void * e1, void * e2), node_t * begin, node_t * end)
{
	...
	while(lbound != rbound->right)
	{
		while(lbound != end->right && cmp_elements(lbound->element, pivot->element) <= 0)
			lbound = lbound->right;

		while(rbound != begin  && cmp_elements(pivot->element, rbound->element) < 0 )
			rbound = rbound->left;

		if (lbound != rbound->right && lbound != end->right)
		{
